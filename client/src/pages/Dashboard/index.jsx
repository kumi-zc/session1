            ...
            {pieData.length === 0 ? <Empty description="No stock data" /> : (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 280 }}>
                <ResponsiveContainer width="55%" height={240}>
                  <PieChart>
                    <Pie data={pieData} cx="50%" cy="50%" innerRadius={55} outerRadius={90} paddingAngle={3} dataKey="value" label={({ name, percent }) => `${(percent * 100).toFixed(0)}%`}>
                ...
                <div style={{ minWidth: 100 }}>
                  {pieData.map((item, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                      ...
                      <Text strong style={{ fontSize: 13, marginLeft: 'auto' }}>{item.value}</Text>
                    </div>
                  ))}
                </div>
              </div>
